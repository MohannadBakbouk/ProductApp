...
final class ActaulCacheManagerTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        configs.inMemoryIdentifier = UUID().uuidString
        cacheManager = CacheManager(configs: configs)
    ...
    func testDeleteProduct() throws {
        let target = products.last!.id
        cacheManager.addBatch(items: products)
        cacheManager.delete(item: products.last!)
        var xxc = (cacheManager.fetch(entity: ProductObject.self) ?? [])
        let isDeleted =  !(cacheManager.fetch(entity: ProductObject.self) ?? []).contains(where: {$0.id == target})
    ...
    func testDeleteAllProducts() throws {
        ...
        let items =  cacheManager.fetch(entity: ProductObject.self)
        XCTAssert((items?.count ?? -1) == 0, "Failed to delete products")
    }
}
